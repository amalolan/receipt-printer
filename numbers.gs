...
function setINR(initial, final){
  if (!initial){
    // if initial doesn't exist, it means there's no need to run this function i.e. the optional input hasn't been selected.
    return
  }
  ...
  try {
    ...
    // If either the initial or final value isn't valid, throws error
    failed("ERROR in cell with amount to convert into words or the cell to write number into words.");
    throw "shown";
  }
}
